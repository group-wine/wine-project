...
cmPlot(linrclass, "red", pred_first = TRUE,
       "Linear Model Classification of Quality for Red Wines")
```
...
cmPlot(linwclass, "white", pred_first = TRUE,
      "Linear Model Classification of Quality for White Wines")
```


## Results: Partial Proportional Odds Model (White Wine)

* Our package includes functions to implement the following models 
...
* For white wine, only the proportional odds model converged 
  ...
  + Kappa: `r round(white.pred.kappa, 2)`
  + Weighted Kappa: `r round(white.pred.wkappa, 2)`
...
cmPlot(red.partial.prop.preds.table, "red", pred_first = T, title = "Partial Proportional Odds Predicted vs Observed")
