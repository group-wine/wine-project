...
* Wine tasting can range range from a casual pastime to a lucrative profession
...
To determine how accurately we can predict expert wine quality ratings using a set of easily measured chemical components of the wine.
...
* The red and white wine datasets were split into training and testing sets
  + 80% of the available data sampled for training models
  + Remaining 20% used to evaluate the accuracy of the trained models
  + Relative frequencies of quality ratings in the full data were preserved in the training and testing data 
* Modeling Approach
  + Machine learning
...
* Reponse takes values in K classes
...
* Computational Efficiency Chalenges

## Methods: Random Forest

* Something here describing the random forest approach

## Variable Selection

* All physiochemical variables were included as potential predictors in random forest models 
...
## Results: Linear Model (red wine quality classification)
...
```
## Results: Linear Model (white wine quality classification)
...
multinomial_red_results <- bind_rows(
  pred_summary_stats(red_test_full_preds, red_test$quality, "Full Model (Linear Terms Only)"),
  pred_summary_stats(red_test_reduced_linear_preds, red_test$quality, "Reduced Model (Linear Terms Only)"),
   pred_summary_stats(red_test_reduced_quad_preds, red_test$quality, "Reduced Model (All Second Order Terms)")
...
multinomial_best_red_model_results <- red_results %>% slice(1) %>% 
  select(-Model)
```
...
multinomial_white_results <- bind_rows(
  pred_summary_stats(white_test_full_preds, white_test$quality, "Full Model (Linear Terms Only)"),
  pred_summary_stats(white_test_linear_preds, white_test$quality, "Reduced Model (Linear Terms Only)"),
   pred_summary_stats(white_test_quad_preds, white_test$quality, "Reduced Model (All Second Order Terms)"))%>% 
  dplyr::arrange(desc(`Weighted Kappa`))
...
multinomial_white_best_model_results <- white_results %>%
  slice(1) %>% 
  select(-Model)
```
...
#weighted kappa
wk.type = round(cohen.kappa(cM$table)$weighted.kappa,4)

results.type <- setNames(data.frame(matrix(ncol = 5, nrow = 1)),
                            c("Prediction Accuracy", "Kappa","Weighted Kappa", "Red", "White"))
results.type[1,] <- c(pred,wk.type,res)
rownames(results.type) = c("Random Forest")

kable(results.type, caption = "Classification Results for Wine Type", booktabs=T) %>%
  add_header_above(c(" ", "Overall Results" = 3, "Percent Correct by Category" = 2)) %>%
...
"RF Classification of Grouped Quality for Red Wines", axis.title.size = 20, axis.text.size = 25)
...
cmPlot(class.white.grp, "white", pred_first = TRUE,
"RF Classification of Grouped Quality for White Wines", axis.title.size = 20, axis.text.size = 25)
```
...
"RF Classification of Grouped Quality for Red Wines \n Subsampled by Group", axis.title.size = 20, axis.text.size = 25)
...
results.white <- setNames(data.frame(matrix(ncol = 10, nrow = 6)),
c("Prediction Accuracy", "Kappa", "Weighted Kappa", "3", "4", "5", "6", "7", "8", "9"))
rownames(results.white) <- c("Random Forest", "Proportional Odds", "Partial Proportional Odds","Non-Proportional Odds", "Multinomial", "Linear Regression")
