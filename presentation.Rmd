...
```{r pressure, echo=FALSE, out.width = '33%', fig.align="center"}
...
* Likelihood based models:
    + Fit the full model and hypothesized that we could find a reduced model. 
    + Determined by correlations between predictors and OLS best subset.
    + Covaraites for reduced model:
...
lllin = function(color,full=FALSE) {
  if (color=='red') {
...
## FULL
linrfull = linrffac = lllin('red',full=TRUE) ; linwfull = linwffac = lllin('white',full=TRUE) 
...
lmresults[1,] = c(linrfacc, round(rfklin$kappa,4), round(rfklin$weighted.kappa,4),linrfpred,NA)
lmresults[2,] = c(linracc, round(rklin$kappa,4), round(rklin$weighted.kappa,4),linrpred,NA)
lmresults[3,] = c(linwfacc, round(wfklin$kappa,4), round(wfklin$weighted.kappa,4),linwfpred)
lmresults[4,] = c(linwacc, round(wklin$kappa,4), round(wklin$weighted.kappa,4),linwpred)
...
```{r varimp_rf, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='.49\\linewidth', fig.show='hold'}
...
* Accuracies 10-15% lower and weighted kappas 0.1 to 0.2 lower
* No clear winner 
    + However, the linear model showed a tendency to predict the mean value. 
    + The multinomial model had higher accuracy for lower and higher quality wines
    + Accounting for the ordered nature of the ratings did not make a difference in predictions 

  
## Discussion: Limitations and Future Directions 

* We speculate some misclassification was due to failure to measure other important chemicals.
* In the future, it would be interesting characterize how individual chemicals relate to wine quality. 
